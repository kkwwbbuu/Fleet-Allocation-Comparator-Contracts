...
st.set_page_config(page_title="Contracts Files Comparator", layout="centered")
st.title("Contracts Files Comparator - Fleet Allocaiton")
...
if uploaded_file1 and uploaded_file2:
    try:
        ...
        for _, row in merged.iterrows():
            ...
            if pd.isna(row["Allocation date_file1"]):
                ...
                if row["Depot Allocation"] != row["Depot"]:
                    ...
                    DepotErrors += 1
                    error_IDs.add(num)
                if row["PM Assign To"] != row["Assigned to"]:
                    problems.append(
                        f"⚠️Fleet Number {num} has different Assigned To "
                        f"({row['PM Assign To']} vs {row['Assigned to']})"
                    )
                    AssignedErrors += 1
                    error_IDs.add(num)
        ...
        for _, row in merged.iterrows():
            ...
            elif row["PM Assign To"] != row["Assigned to"]:
                analytics_data.append({
                    "Fleet Number": num,
                    "Depot in SAP" : row["Depot Allocation"],
                    "Assigned To in SAP" : row["PM Assign To"],
        ...
        filename = st.text_input("File name for download", value="Contracts_fleet_allocation_comparison_result.xlsx")
